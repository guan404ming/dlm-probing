...
Suppressing \texttt{f15601} or the top-5 fail features within the plateau (step 64), and the reverse intervention ($\alpha{=}-5$), each yield $0/202$ label flips, as does a counterfactual replacement that simultaneously subtracts the top-5 fail-enriched and adds the top-5 pass-enriched directions (Appendix~\ref{sec:appendix_effect}).
...
Pairing the fail-out and pass-in directions therefore does not unlock the controllability that plain suppression failed to demonstrate, further supporting the diagnostic-only interpretation.
