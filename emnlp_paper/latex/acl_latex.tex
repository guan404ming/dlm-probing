...
Suppressing \texttt{f15601} or the top-5 fail features within the plateau (step 64), and the reverse intervention ($\alpha{=}-5$), each yield $0/202$ label flips, as does a counterfactual replacement that subtracts the top-5 fail-enriched and adds the top-5 pass-enriched directions. A broader sweep of $17$ single-layer linear protocols on small $n$ (Appendix~\ref{sec:appendix_effect}) varying magnitude ($\alpha$ up to $50$), direction, start step, feature-set size, and replacement vs.\ suppression yields $0/255$ fail$\to$pass across the entire family.
Only early intervention from step 16, before plateau formation, produces a marginal effect ($3/202\!\approx\!1.5\%$) under the main protocol.
...
Every one of the $17$ protocols yields $0/15$ fail$\to$pass on cluster-1, for an aggregate $0/255$ across the entire single-layer linear sweep; only one protocol (start step $s4$, $\alpha\!=\!5$) induces a single pass-side regression ($1/5$).
This bounds the negative result tightly to the single-layer linear family at L26 over a wide swath of magnitudes, directions, start steps, and feature sets. Whether gradient-based, multi-layer joint, or non-linear interventions can flip correctness in DLMs remains open.
