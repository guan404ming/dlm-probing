...
\paragraph{What the plateau features represent.}
By step 64, LLaDA has unmasked blocks 0--3 (signature and early body), so code and structural failure modes are exposed while later tokens remain uncertain; reasoning tasks only peak at the final checkpoint.
Activation-maximising examples at step 64 (Appendix~\ref{sec:appendix_actmax}) further show that \texttt{f15601} fires on complex-math docstring contexts (a task-difficulty correlate, not a fail-cause), whereas late-state \texttt{f11265} fires on broken-syntax contexts (closer to a true failure detector); this dissociation explains the within-plateau steering null, since ablating \texttt{f15601} perturbs a difficulty signal upstream of the failure mechanism.
...
Code and structural tasks form a mid-denoising plateau, whereas reasoning tasks peak at the final state, with cross-model agreement on three of four tasks and a Fisher-combined $p\!<\!10^{-7}$ on LLaDA-MBPP anchoring the trajectory claim.
...
The plateau features therefore behave as robust diagnostic markers; causal control over correctness in DLMs likely requires stronger, earlier, or multi-layer interventions than the single-layer linear protocol tested here.
...
\begin{figure}[H]
  ...
  \caption{LLaDA-MBPP signal-to-null gap across SAE layers at plateau steps 64 and 68. L26 is the unique layer where the gap reaches permutation $p<0.05$ (step 68, gap $+0.357$, $p=0.006$). Top-1 fail-enriched feature varies by layer; \texttt{f15601} appears only at L26.}
...
\begin{figure}[H]
  ...
  \caption{Fisher combined $p$ across the sparse $5$-step grid for each of the $8$ (DLM, task) cells. Dream-JSON reaches aggregated significance ($p=0.047$); LLaDA-MBPP is next strongest. The dashed line marks $p=0.05$.}
  \label{fig:fisher}
\end{figure}
...
\section{Activation-Maximizing Examples}
