...
\paragraph{Why linear steering fails: testing two hypotheses.}
Two structural explanations can account for the $0/345$ within-plateau null. (\emph{H1, sparse-subspace}) the SAE captures only $85$--$95\%$ of the raw-probe AUC, so the remaining $5$--$15\%$ may be the causal portion sparse interventions miss. (\emph{H2, block-commit timing}) blocks 0--3 commit by step $\sim\!48$, so any L26 perturbation at step 64 alters at best blocks 4--7, not the function signature where MBPP correctness is most often decided. H1 predicts raw-direction steering should succeed; H2 predicts it should still fail. The raw-direction sweep (Appendix~\ref{sec:appendix_effect}) refutes H1: mean-difference, top-PC, and Fisher LDA all yield $0/90$ fail$\to$pass even at $\|\Delta h\|\!>\!17\!\times\!\|h\|$. H2 remains consistent with the only nonzero flips (step 16, pre-commit) and with activation-maximising contexts where \texttt{f15601} fires inside the committed signature region (Appendix~\ref{sec:appendix_actmax}).
